...
export class EmergencyResourceTableComponent {
  ...
  filteredResources = computed<EmergencyResource[]>(() => {
    let temporal = this.resources().value() ?? [];
    const orderBy = this.orderBy();

    if (orderBy === 'newest') {
      temporal = temporal.sort((a, b) =>
    ...
    // Access count sorting removed as it's not in the current model

    if (this.search().length > 0) {
      temporal = temporal.filter((resource) =>
        resource.name.toLowerCase().includes(this.search().toLowerCase()),
      );
    }
  ...
  resources = input.required<ResourceRef<EmergencyResource[] | undefined>>();
