...
@Component({
  ...
  template: `
    <div class="relative w-full h-96">
      ...
      @if (isLoading()) {
        <div
          class="absolute inset-0 flex items-center justify-center bg-base-100/50 z-10 rounded"
        >
      ...
      <div class="absolute top-2 right-2 z-10">
        <div class="relative">
          ...
          @if (showSuggestions && suggestions().length > 0) {
            <div
              class="absolute mt-1 w-full max-h-60 overflow-y-auto bg-base-100 rounded-md shadow-lg z-20 border border-base-300"
            >
...
export class MapCenterComponent implements AfterViewInit {
      ...
      .then((data) => {
        ...
        return data.map((item) => {
          ...
          if (
            ...
            (item.display_name &&
              item.display_name.toLowerCase().includes('universidad'))
          ) {
            icon = 'school';
          }
  ...
  private initializeMap(): void {
    ...
    try {
      ...
      const cartoDBTiles = L.tileLayer(
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        if (
          ...
          alert(
            'No se encontraron centros de salud en esta área. Intenta acercar el mapa o buscar en otra ubicación.',
          );
          return;
        }
  ...
  private createHealthCenterPopup(item: any): string {
    ...
    const phone =
      item.tags?.phone || item.tags?.['contact:phone'] || 'No disponible';
  ...
  searchLocation(query: string): void {
    ...
    fetch(
      `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(searchQuery)}&format=json&countrycodes=pe&addressdetails=1&limit=1`,
    )
      .then((response) => response.json())
      .then((data) => {
        this.isLoading.set(false);
        ...
        if (data && data.length > 0) {
          ...
          alert(
            'No se encontró la ubicación. Intente con una búsqueda más específica.',
          );
        }
      })
      .catch((err) => {
