...
export default class DashboardPageComponent {
  ...
  healthCentersChange = computed(() => {
    const stats = this.dashboardStats();
    return stats && stats.healthCenters
      ? stats.healthCenters.percentageChange
      : 0;
  });

  activeTab = signal('Blog Posts');
  ...
  constructor() {
    effect(() => {
      ...
      if (
        moodStatsData &&
        Array.isArray(moodStatsData) &&
        moodStatsData.length > 0
      ) {
    ...
    effect(() => {
      ...
      if (
        blogStatsData &&
        Array.isArray(blogStatsData) &&
        blogStatsData.length > 0
      ) {
