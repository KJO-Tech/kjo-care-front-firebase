...
export default class SettingMoodStateComponent {
  ...
  readonly moodStateResource = rxResource({
    ...
    loader: () =>
      this.isMoodStateEmpty(this.moodStateSignal())
        ? NEVER
        : this.moodStateService.addMoodState(this.moodStateSignal()),
  });
  ...
  editingMood = signal<Mood | null>(null);
  ...
  handleSaveMood(mood: Mood): void {
    if (this.isNewMood()) {
      this.moodStateService
        .addMoodState(mood as unknown as MoodStateRequest)
        .subscribe({
          next: () => {
            this.loadMoodTracking.reload();
            this.cancelEdit();
      ...
      this.moodStateService.updateMoodState(mood.id, mood).subscribe({
        next: () => {
          this.loadMoodTracking.reload();
          this.cancelEdit();
          this.toastService.addToast({
